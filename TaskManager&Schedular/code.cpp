...
bool dateEarlier(const string &a, const string &b)
{
    ...
    if (!parseDate(a, da, ma, ya))
        return false;
    if (!parseDate(b, db, mb, yb))
        return false;
    if (ya != yb)
        return ya < yb;
    if (ma != mb)
...
{
    int dd, dm, dy;
    if (!parseDate(deadline, dd, dm, dy))
        return -1;
...
public:
    ...
    void displayCompact(int index) const
    {
        ...
        string dl = deadline;
        if (status == 2)
            dl = string(RED) + BOLD + deadline + " !!" + RESET;
        else if (status == 1)
            dl = string(YELLOW) + BOLD + deadline + " today" + RESET;
...
public:
    ...
    void viewByPriority() const
    {
        ...
        string label = (priority == "H") ? "HIGH" : (priority == "M") ? "MEDIUM"
                                                                      : "LOW";
        cout << "\n"
        ...
        if (!found)
            cout << "No tasks with " << label << " priority.\n";
    }
    ...
    void editTask()
    {
        ...
        if (val == "H" || val == "M" || val == "L")
            ...
            cout << YELLOW << "  Invalid priority kept unchanged.\n"
                 << RESET;
        ...
        {
            int dd, dm, dy;
            if (parseDate(val, dd, dm, dy))
                t.setDeadline(val);
            else
                cout << YELLOW << "  Invalid date format, kept unchanged.\n"
                     << RESET;
        }
    ...
    void showStats() const
    {
        ...
        if (overdue > 0 || dueToday > 0)
        {
            cout << "\n";
            if (overdue > 0)
                cout << RED << BOLD << "  !! Overdue:    " << RESET << overdue << "\n";
            if (dueToday > 0)
                cout << YELLOW << BOLD << "  !! Due today:  " << RESET << dueToday << "\n";
        }
    ...
    void loadFromFile()
    {
        ...
        while (getline(file, line))
        {
            ...
            if (!getline(ss, deadline, '|'))
                continue;
            if (!getline(ss, completedStr, '|') && !getline(ss, completedStr))
                continue;
